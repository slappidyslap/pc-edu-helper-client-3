...
function onChangeSubject(event: Event) {
  ...
  emit('changeSubject', input.value)
}

function onChangeTarget(event: Event) {
  const input = event.target as HTMLInputElement
  emit('changeTarget', input.value)
}

function onChangeTeacher(event: Event) {
  const input = event.target as HTMLInputElement
  emit('changeTeacher', input.value)
}

function onChangeAudience(event: Event) {
  const input = event.target as HTMLInputElement
  emit('changeAudience', input.value)
}
...
<template>
  ...
  <input v-model="lesson.teacher" @change="onChangeTeacher" :class="lesson.isTeacherValid ? 'border-l w-5/12' : 'border-l w-5/12 not-valid-teacher-input'" type="text">
